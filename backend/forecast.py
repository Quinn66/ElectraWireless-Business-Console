...
def train_prophet_model(train_target: TimeSeries, train_covariates: TimeSeries = None) -> DartsProphet:
    ...
    model = DartsProphet(
        changepoint_prior_scale= 0.02,  # default ~0.05 →lower = smoother
...
def run_prophet_forecast(quarters: int, company_id: str = "CMP_001") -> List[Dict]:
    ...
    try:
        ...
        # Generate forecast
        forecast = generate_forecast(model, quarters)
...
if __name__ == "__main__":
    ...
    try:
        results = main_forecasting_pipeline(company_id="CMP_001", test_size=0.2)
